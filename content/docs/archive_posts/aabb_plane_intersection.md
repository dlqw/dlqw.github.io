...
起因是我在学习 runtime 的 mesh 切割算法,需要判断目标 mesh 是否和切面相交(不相交的话我就不需要去处理了)
...
3. 下一段代码计算了 AABB 的投影半径,这个半径是 AABB 投影到平面法线上的最大距离。
   其取 AABB 的范围和平面法线的绝对值的点积,用于确保计算出的长度是正值,因为两个向量中的元素都是正值或零。
...
4. 下面计算了 AABB 中心点到平面的距离,计为 s
5. 最后的返回值即为 AABB 自身在平面法线上的距离和其中心到平面距离长短的判断
...
- [AABB - Axis Aligned Bounding Box](https://www.google.com/search?q=AABB+-+Axis+Aligned+Bounding+Box)
- [Bounds-Unity_Documentation](https://docs.unity3d.com/ScriptReference/Bounds.html)
